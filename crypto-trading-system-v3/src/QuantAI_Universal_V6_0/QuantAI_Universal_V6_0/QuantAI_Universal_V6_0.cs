// =====================================================================================================
//  QuantAI_Universal_V6_0 (версия 6.0) — адаптивный AI-cBot для cTrader (API 4.x / cTrader.Automate)
//
//  ИСТОРИЯ ВЕРСИЙ
...
namespace cAlgo.Robots
{
    ...
    internal sealed class BarAnalysis
    {
        ...
        public readonly StrategyVote[] Votes = new StrategyVote[QuantAI_Universal_V6_0.StrategyCount];
    ...
    public class QuantAI_Universal_V6_0 : Robot
    {
        ...
        [Parameter("Доп. инструменты", Group = "Потоки", DefaultValue = "GBPUSD, USDJPY, XAUUSD, US500|SPX500, NAS100|USTEC, GER40|GER30|DE40, BTCUSD, ETHUSD")]
        ...
        [Parameter("Группы связанных инструментов", Group = "Потоки", DefaultValue = "EURUSD GBPUSD; US500 SPX500 NAS100 USTEC US30; BTCUSD ETHUSD")]
        ...
        [Parameter("Закрытие не-крипто в пятницу (UTC)", Group = "Фильтры", DefaultValue = "19:45")]
        ...
        private TimeSpan _weekendClose;
        private bool _weekendCloseEnabled;
        ...
        protected override void OnStart()
        {
            ...
            Print("QuantAI_Universal версия " + BotVersion + " запущена. Потоков: " + _streams.Count + " — " + names +
                  ...
                  (_tradeHoursEnabled ? TradeHoursStart + "–" + TradeHoursEnd + " UTC" : "в любое время работы рынка") +
                  (_weekendCloseEnabled ? ", закрытие в пятницу в " + WeekendCloseTime + " UTC" : "") + "; крипта — 24/7.");
        }
        ...
        private void ValidateGlobalSettings()
        {
            ...
            _weekendCloseEnabled = TryParseTime(WeekendCloseTime, out _weekendClose);
            if (!_weekendCloseEnabled && !string.IsNullOrWhiteSpace(WeekendCloseTime))
            {
                Print("ВНИМАНИЕ: не удалось прочитать время закрытия в пятницу ('" + WeekendCloseTime + "'). Формат ЧЧ:ММ. Закрытие перед выходными отключено.");
            }
        ...
        /// <summary>Пятница после «Закрытия не-крипто»: позиции форекса, металлов и индексов закрываются до выходных.</summary>
        internal bool IsWeekendCloseTime(DateTime utc) =>
            _weekendCloseEnabled &&
            ((utc.DayOfWeek == DayOfWeek.Friday && utc.TimeOfDay >= _weekendClose) || utc.DayOfWeek == DayOfWeek.Saturday);
    ...
    internal sealed class TradingStream
    {
        ...
        private readonly QuantAI_Universal_V6_0 _r;
        ...
        private readonly EdgeTracker[] _edge = new EdgeTracker[QuantAI_Universal_V6_0.StrategyCount];
        ...
        public TradingStream(QuantAI_Universal_V6_0 robot, Bars bars, Symbol symbol, string symbolName, TimeFrame timeFrame)
        {
            _r = robot;
            _bars = bars;
            _symbol = symbol;
        ...
        public void Start()
        {
            ...
            _legacyLabels = new string[QuantAI_Universal_V6_0.LegacyLabels.Length];
            for (int k = 0; k < _legacyLabels.Length; k++) _legacyLabels[k] = QuantAI_Universal_V6_0.LegacyLabels[k] + " " + ShortTimeFrame();

            for (int s = 0; s < QuantAI_Universal_V6_0.StrategyCount; s++) _edge[s] = new EdgeTracker(EvidenceWindow);
            ...
            _storageKey = StorageKey(QuantAI_Universal_V6_0.MemoryPrefix + " " + _symbolName + " " + _timeFrame);
            if (_r.ResetAiMemory)
            {
        ...
        private DateTime _lastWeekendCloseAttempt = DateTime.MinValue;

        /// <summary>Пятница вечером: позиции форекса, металлов и индексов закрываются, чтобы не держать риск гэпа через выходные.</summary>
        private void CloseBeforeWeekendIfNeeded()
        {
            if (QuantAI_Universal_V6_0.IsCrypto(_symbolName)) return;
            DateTime now = _r.Server.TimeInUtc;
            if (!_r.IsWeekendCloseTime(now) || (now - _lastWeekendCloseAttempt).TotalSeconds < 60) return;

            List<Position> own = OwnPositions();
            if (own.Count == 0) return;
            _lastWeekendCloseAttempt = now;

            foreach (Position p in own)
            {
                double profit = p.NetProfit;
                TradeResult result = _r.ClosePosition(p);
                Log("WEEKEND CLOSE " + _symbolName + " " + p.Id + ": " + (result.IsSuccessful
                    ? "закрыта перед выходными, результат " + profit.ToString("F2", CultureInfo.InvariantCulture)
        ...
        private double RegimeFactor(int s, double pTrend) => QuantAI_Universal_V6_0.IsTrendStrategy[s] ? pTrend : 1.0 - pTrend;
        ...
        private void ComputeEnsemble(BarAnalysis a, bool[] allowed = null)
        {
            ...
            for (int s = 0; s < QuantAI_Universal_V6_0.StrategyCount; s++)
            {
        ...
        private void OpenShadowTrade(BarAnalysis a)
        {
            ...
            t.VoteStrengths = new double[QuantAI_Universal_V6_0.StrategyCount];
            for (int s = 0; s < QuantAI_Universal_V6_0.StrategyCount; s++)
            {
        ...
        {
            EdgeStats st = Edge(s);
            string name = QuantAI_Universal_V6_0.StrategyNames[s];
        ...
        private void TryEnter(BarAnalysis a, DateTime nowUtc)
        {
            ...
            if (!_r.InTradingHours(_symbolName, nowUtc))
            {
                RegisterSkip(SkipReason.OffHours, "вне торговых часов " + _r.TradeHoursStart + "–" + _r.TradeHoursEnd +
                    " UTC: форекс, металлы и индексы торгуются только в ликвидные часы Лондона и Нью-Йорка", a);
                return;
            }

            if (!QuantAI_Universal_V6_0.IsCrypto(_symbolName) && _r.IsWeekendCloseTime(nowUtc))
            {
                RegisterSkip(SkipReason.OffHours, "пятница после " + _r.WeekendCloseTime + " UTC — перед выходными новые сделки не открываются", a);
                return;
            }
            ...
            var tiers = new StrategyTier[QuantAI_Universal_V6_0.StrategyCount];
            var allowed = new bool[QuantAI_Universal_V6_0.StrategyCount];
            ...
            var votes = new double[QuantAI_Universal_V6_0.StrategyCount];
            for (int s = 0; s < QuantAI_Universal_V6_0.StrategyCount; s++)
            {
                if (allowed != null && !allowed[s]) continue;
        ...
        public void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            if (m.Features != null && m.VoteStrengths != null)
            {
                ...
                for (int s = 0; s < QuantAI_Universal_V6_0.StrategyCount; s++)
                {
                    ...
                    changes.Append(QuantAI_Universal_V6_0.StrategyNames[s]).Append(' ').Append(F(before)).Append("→").Append(F(_realWeights[s])).Append("; ");
                }
        ...
        public void SaveMemory()
        {
            try
            {
                ...
                for (int s = 0; s < QuantAI_Universal_V6_0.StrategyCount; s++) sb.Append(R(_realWeights[s])).Append(';');
        ...
        private void LoadMemory()
        {
            try
            {
                ...
                int expected = 1 + QuantAI_Universal_V6_0.StrategyCount + 2 + (2 * NaiveBayesModel.FeatureCount * 3) + 2;
                ...
                int idx = 1;
                for (int s = 0; s < QuantAI_Universal_V6_0.StrategyCount; s++) _realWeights[s] = Math.Max(0.25, Math.Min(3.0, P(parts[idx++], 1.0)));
            ...
            {
                _nbReal.Clear();
                for (int s = 0; s < QuantAI_Universal_V6_0.StrategyCount; s++) _realWeights[s] = 1.0;
        ...
        private string VotesText(BarAnalysis a)
        {
            ...
            for (int s = 0; s < QuantAI_Universal_V6_0.StrategyCount; s++)
            {
                ...
                sb.Append(QuantAI_Universal_V6_0.StrategyNames[s]).Append(v.Direction > 0 ? " ▲" : " ▼").Append(F(v.Strength)).Append("×w").Append(F(EffectiveWeight(s)));
            }
        ...
        private string WeightsText()
        {
            ...
            {
                if (s > 0) sb.Append(", ");
                sb.Append(QuantAI_Universal_V6_0.StrategyNames[s]).Append(' ').Append(F(EffectiveWeight(s)));
            }
            return sb.ToString();
        }
