// =====================================================================================================
//  QuantAI_Universal_V5_7_Test (версия 5.7-TEST) — адаптивный AI-cBot для cTrader (API 4.x / cTrader.Automate)
//
//  ИСТОРИЯ ВЕРСИЙ
//   5.7-TEST — защита маржи надёжнее: в 5.6 залог считался только оценкой брокера (GetEstimatedMargin),
//         а на демо сделки открывались с залогом больше лимита (0.08 BTC и 0.58 лота EURUSD). Теперь
//         берётся САМАЯ БОЛЬШАЯ из трёх оценок: брокера, собственного расчёта (стоимость позиции в валюте
...
namespace cAlgo.Robots
{
    ...
    internal sealed class BarAnalysis
    {
        ...
        public readonly StrategyVote[] Votes = new StrategyVote[QuantAI_Universal_V5_7_Test.StrategyCount];
    ...
    internal sealed class TradingStream
    {
        ...
        private readonly QuantAI_Universal_V5_7_Test _r;
        ...
        public TradingStream(QuantAI_Universal_V5_7_Test robot, Bars bars, Symbol symbol, string symbolName, TimeFrame timeFrame)
        {
            _r = robot;
            _bars = bars;
            _symbol = symbol;
        ...
        private double RegimeFactor(int s, double pTrend) => QuantAI_Universal_V5_7_Test.IsTrendStrategy[s] ? pTrend : 1.0 - pTrend;
        ...
        private void ComputeEnsemble(BarAnalysis a)
        {
            ...
            for (int s = 0; s < QuantAI_Universal_V5_7_Test.StrategyCount; s++)
            {
        ...
        private void OpenShadowTrade(BarAnalysis a)
        {
            ...
            var votes = new double[QuantAI_Universal_V5_7_Test.StrategyCount];
            for (int s = 0; s < QuantAI_Universal_V5_7_Test.StrategyCount; s++)
            {
        ...
        /// большая из трёх оценок (см. EstimateMargin): одной оценке брокера в 5.6 доверять оказалось
        /// нельзя — сделки открывались с залогом больше лимита. Объём уменьшается пропорционально; если
        ...
        private bool ApplyMarginLimit(TradeType type, double slPips, ref double volume, ref double riskMoney, ref string note)
        {
            ...
            if (margin > budget)
            {
                double scaled = budget > 0 ? _symbol.NormalizeVolumeInUnits(volume * budget / margin, RoundingMode.Down) : 0;
                if (scaled < _symbol.VolumeInUnitsMin)
                {
                ...
                margin = EstimateMargin(type, volume, out detail);
        ...
        public void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            if (m.Features != null && m.VoteStrengths != null)
            {
                ...
                for (int s = 0; s < QuantAI_Universal_V5_7_Test.StrategyCount; s++)
                {
                    ...
                    changes.Append(QuantAI_Universal_V5_7_Test.StrategyNames[s]).Append(' ').Append(F(before)).Append("→").Append(F(_realWeights[s])).Append("; ");
                }
        ...
        public void SaveMemory()
        {
            try
            {
                ...
                for (int s = 0; s < QuantAI_Universal_V5_7_Test.StrategyCount; s++) sb.Append(R(_realWeights[s])).Append(';');
        ...
        private void LoadMemory()
        {
            try
            {
                ...
                int expected = 1 + QuantAI_Universal_V5_7_Test.StrategyCount + 2 + (2 * NaiveBayesModel.FeatureCount * 3) + 2;
                ...
                int idx = 1;
                for (int s = 0; s < QuantAI_Universal_V5_7_Test.StrategyCount; s++) _realWeights[s] = Math.Max(0.25, Math.Min(3.0, P(parts[idx++], 1.0)));
            ...
            {
                _nbReal.Clear();
                for (int s = 0; s < QuantAI_Universal_V5_7_Test.StrategyCount; s++) _realWeights[s] = 1.0;
        ...
        private string VotesText(BarAnalysis a)
        {
            ...
            for (int s = 0; s < QuantAI_Universal_V5_7_Test.StrategyCount; s++)
            {
                ...
                sb.Append(QuantAI_Universal_V5_7_Test.StrategyNames[s]).Append(v.Direction > 0 ? " ▲" : " ▼").Append(F(v.Strength)).Append("×w").Append(F(EffectiveWeight(s)));
            }
        ...
        private string WeightsText()
        {
            ...
            {
                if (s > 0) sb.Append(", ");
                sb.Append(QuantAI_Universal_V5_7_Test.StrategyNames[s]).Append(' ').Append(F(EffectiveWeight(s)));
            }
            return sb.ToString();
        }
