// =====================================================================================================
//  QuantAI_Universal_V5_8_Test (версия 5.8-TEST) — адаптивный AI-cBot для cTrader (API 4.x / cTrader.Automate)
//
//  ИСТОРИЯ ВЕРСИЙ
...
namespace cAlgo.Robots
{
    ...
    internal sealed class BarAnalysis
    {
        ...
        public readonly StrategyVote[] Votes = new StrategyVote[QuantAI_Universal_V5_8_Test.StrategyCount];
    ...
    internal sealed class TradingStream
    {
        ...
        private readonly QuantAI_Universal_V5_8_Test _r;
        ...
        public TradingStream(QuantAI_Universal_V5_8_Test robot, Bars bars, Symbol symbol, string symbolName, TimeFrame timeFrame)
        {
            _r = robot;
            _bars = bars;
            _symbol = symbol;
        ...
        private double RegimeFactor(int s, double pTrend) => QuantAI_Universal_V5_8_Test.IsTrendStrategy[s] ? pTrend : 1.0 - pTrend;
        ...
        private void ComputeEnsemble(BarAnalysis a)
        {
            ...
            for (int s = 0; s < QuantAI_Universal_V5_8_Test.StrategyCount; s++)
            {
        ...
        private void OpenShadowTrade(BarAnalysis a)
        {
            ...
            var votes = new double[QuantAI_Universal_V5_8_Test.StrategyCount];
            for (int s = 0; s < QuantAI_Universal_V5_8_Test.StrategyCount; s++)
            {
        ...
        public void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            if (m.Features != null && m.VoteStrengths != null)
            {
                ...
                for (int s = 0; s < QuantAI_Universal_V5_8_Test.StrategyCount; s++)
                {
                    ...
                    changes.Append(QuantAI_Universal_V5_8_Test.StrategyNames[s]).Append(' ').Append(F(before)).Append("→").Append(F(_realWeights[s])).Append("; ");
                }
        ...
        public void SaveMemory()
        {
            try
            {
                ...
                for (int s = 0; s < QuantAI_Universal_V5_8_Test.StrategyCount; s++) sb.Append(R(_realWeights[s])).Append(';');
        ...
        private void LoadMemory()
        {
            try
            {
                ...
                int expected = 1 + QuantAI_Universal_V5_8_Test.StrategyCount + 2 + (2 * NaiveBayesModel.FeatureCount * 3) + 2;
                ...
                int idx = 1;
                for (int s = 0; s < QuantAI_Universal_V5_8_Test.StrategyCount; s++) _realWeights[s] = Math.Max(0.25, Math.Min(3.0, P(parts[idx++], 1.0)));
            ...
            {
                _nbReal.Clear();
                for (int s = 0; s < QuantAI_Universal_V5_8_Test.StrategyCount; s++) _realWeights[s] = 1.0;
        ...
        private string VotesText(BarAnalysis a)
        {
            ...
            for (int s = 0; s < QuantAI_Universal_V5_8_Test.StrategyCount; s++)
            {
                ...
                sb.Append(QuantAI_Universal_V5_8_Test.StrategyNames[s]).Append(v.Direction > 0 ? " ▲" : " ▼").Append(F(v.Strength)).Append("×w").Append(F(EffectiveWeight(s)));
            }
        ...
        private string WeightsText()
        {
            ...
            {
                if (s > 0) sb.Append(", ");
                sb.Append(QuantAI_Universal_V5_8_Test.StrategyNames[s]).Append(' ').Append(F(EffectiveWeight(s)));
            }
            return sb.ToString();
        }
