// =====================================================================================================
//  QuantAI_Universal_V6_1 (версия 6.1) — адаптивный AI-cBot для cTrader (API 4.x / cTrader.Automate)
//
//  ИСТОРИЯ ВЕРСИЙ
...
namespace cAlgo.Robots
{
    ...
    internal sealed class BarAnalysis
    {
        ...
        public readonly StrategyVote[] Votes = new StrategyVote[QuantAI_Universal_V6_1.StrategyCount];
    ...
    public class QuantAI_Universal_V6_1 : Robot
    {
        ...
        protected override void OnStart()
        {
            ...
            Print("QuantAI_Universal версия " + BotVersion + " запущена. Потоков: " + _streams.Count + " — " + names +
                  ...
                  (WeekendCloseMinutes > 0 ? ", позиции закрываются за " + WeekendCloseMinutes + " мин до закрытия рынка на выходные/праздник" : "") +
    ...
    internal sealed class TradingStream
    {
        ...
        private readonly QuantAI_Universal_V6_1 _r;
        ...
        private readonly EdgeTracker[] _edge = new EdgeTracker[QuantAI_Universal_V6_1.StrategyCount];
        ...
        public TradingStream(QuantAI_Universal_V6_1 robot, Bars bars, Symbol symbol, string symbolName, TimeFrame timeFrame)
        {
            _r = robot;
            _bars = bars;
            _symbol = symbol;
        ...
        public void Start()
        {
            ...
            _legacyLabels = new string[QuantAI_Universal_V6_1.LegacyLabels.Length];
            for (int k = 0; k < _legacyLabels.Length; k++) _legacyLabels[k] = QuantAI_Universal_V6_1.LegacyLabels[k] + " " + ShortTimeFrame();

            for (int s = 0; s < QuantAI_Universal_V6_1.StrategyCount; s++) _edge[s] = new EdgeTracker(EvidenceWindow);
            ...
            _storageKey = StorageKey(QuantAI_Universal_V6_1.MemoryPrefix + " " + _symbolName + " " + _timeFrame);
            if (_r.ResetAiMemory)
            {
        ...
        /// <summary>
        /// Рынок инструмента закрывается в пределах window и потом будет закрыт не меньше 6 часов (выходные,
        /// праздник) — по расписанию брокера. Короткие ежедневные перерывы сюда не попадают. Крипта — никогда.
        /// </summary>
        private bool LongClosureWithin(TimeSpan window)
        {
            if (QuantAI_Universal_V6_1.IsCrypto(_symbolName) || window <= TimeSpan.Zero) return false;
            try
            {
                MarketHours hours = _symbol.MarketHours;
                if (!hours.IsOpened()) return false;
                TimeSpan left = hours.TimeTillClose();
                if (left <= TimeSpan.Zero || left > window) return false;
                DateTime closeAt = _r.Server.Time + left;
        ...
        /// <summary>Перед выходными и праздниками позиции форекса, металлов и индексов закрываются — без риска гэпа.</summary>
        private void CloseBeforeWeekendIfNeeded()
        {
            if (_r.WeekendCloseMinutes <= 0 || QuantAI_Universal_V6_1.IsCrypto(_symbolName)) return;
            DateTime now = _r.Server.TimeInUtc;
            if ((now - _lastWeekendCloseAttempt).TotalSeconds < 60) return;
            if (!LongClosureWithin(TimeSpan.FromMinutes(_r.WeekendCloseMinutes))) return;
        ...
        private BarAnalysis ProcessClosedBar(int i)
        {
            ...
            bool rolloverPause = !QuantAI_Universal_V6_1.IsCrypto(_symbolName) && _r.IsSessionBlocked(entryTime);
        ...
        private double RegimeFactor(int s, double pTrend) => QuantAI_Universal_V6_1.IsTrendStrategy[s] ? pTrend : 1.0 - pTrend;
        ...
        private void ComputeEnsemble(BarAnalysis a, bool[] allowed = null)
        {
            ...
            for (int s = 0; s < QuantAI_Universal_V6_1.StrategyCount; s++)
            {
        ...
        private void OpenShadowTrade(BarAnalysis a)
        {
            ...
            t.VoteStrengths = new double[QuantAI_Universal_V6_1.StrategyCount];
            for (int s = 0; s < QuantAI_Universal_V6_1.StrategyCount; s++)
            {
        ...
        {
            EdgeStats st = Edge(s);
            string name = QuantAI_Universal_V6_1.StrategyNames[s];
        ...
        private void TryEnter(BarAnalysis a, DateTime nowUtc)
        {
            ...
            if (!QuantAI_Universal_V6_1.IsCrypto(_symbolName) && _r.IsSessionBlocked(nowUtc))
            {
            ...
            var tiers = new StrategyTier[QuantAI_Universal_V6_1.StrategyCount];
            var allowed = new bool[QuantAI_Universal_V6_1.StrategyCount];
            ...
            var votes = new double[QuantAI_Universal_V6_1.StrategyCount];
            for (int s = 0; s < QuantAI_Universal_V6_1.StrategyCount; s++)
            {
                if (allowed != null && !allowed[s]) continue;
        ...
        public void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            if (m.Features != null && m.VoteStrengths != null)
            {
                ...
                for (int s = 0; s < QuantAI_Universal_V6_1.StrategyCount; s++)
                {
                    ...
                    changes.Append(QuantAI_Universal_V6_1.StrategyNames[s]).Append(' ').Append(F(before)).Append("→").Append(F(_realWeights[s])).Append("; ");
                }
        ...
        public void SaveMemory()
        {
            try
            {
                ...
                for (int s = 0; s < QuantAI_Universal_V6_1.StrategyCount; s++) sb.Append(R(_realWeights[s])).Append(';');
        ...
        private void LoadMemory()
        {
            try
            {
                ...
                int expected = 1 + QuantAI_Universal_V6_1.StrategyCount + 2 + (2 * NaiveBayesModel.FeatureCount * 3) + 2;
                ...
                int idx = 1;
                for (int s = 0; s < QuantAI_Universal_V6_1.StrategyCount; s++) _realWeights[s] = Math.Max(0.25, Math.Min(3.0, P(parts[idx++], 1.0)));
            ...
            {
                _nbReal.Clear();
                for (int s = 0; s < QuantAI_Universal_V6_1.StrategyCount; s++) _realWeights[s] = 1.0;
        ...
        private string VotesText(BarAnalysis a)
        {
            ...
            for (int s = 0; s < QuantAI_Universal_V6_1.StrategyCount; s++)
            {
                ...
                sb.Append(QuantAI_Universal_V6_1.StrategyNames[s]).Append(v.Direction > 0 ? " ▲" : " ▼").Append(F(v.Strength)).Append("×w").Append(F(EffectiveWeight(s)));
            }
        ...
        private string WeightsText()
        {
            ...
            {
                if (s > 0) sb.Append(", ");
                sb.Append(QuantAI_Universal_V6_1.StrategyNames[s]).Append(' ').Append(F(EffectiveWeight(s)));
            }
            return sb.ToString();
        }
