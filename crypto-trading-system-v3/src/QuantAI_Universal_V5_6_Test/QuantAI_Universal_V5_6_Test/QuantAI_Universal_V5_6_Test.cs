// =====================================================================================================
//  QuantAI_Universal_V5_6_Test (версия 5.1) — адаптивный AI-cBot для cTrader (API 4.x / cTrader.Automate)
//
//  ИСТОРИЯ ВЕРСИЙ
...
namespace cAlgo.Robots
{
    ...
    internal sealed class BarAnalysis
    {
        ...
        public readonly StrategyVote[] Votes = new StrategyVote[QuantAI_Universal_V5_6_Test.StrategyCount];
    ...
    public class QuantAI_Universal_V5_6_Test : Robot
    {
        ...
        [Parameter("Доп. таймфреймы", Group = "Потоки", DefaultValue = "m15")]
        public string ExtraTimeFrames { get; set; }

        [Parameter("Доп. инструменты", Group = "Потоки", DefaultValue = "XAUUSD, BTCUSD, ETHUSD")]
        public string ExtraSymbols { get; set; }

        [Parameter("Макс. позиций всего", Group = "Потоки", DefaultValue = 4, MinValue = 1, MaxValue = 20)]
        public int MaxTotalPositions { get; set; }

        // ----- Параметры: AI и пороги --------------------------------------------------------------

        [Parameter("Мин. уверенность режима", Group = "AI и пороги", DefaultValue = 0.05, MinValue = 0.0, MaxValue = 1.0, Step = 0.01)]
        public double MinRegimeConfidence { get; set; }

        [Parameter("Мин. уверенность ансамбля", Group = "AI и пороги", DefaultValue = 0.10, MinValue = 0.0, MaxValue = 1.0, Step = 0.01)]
        public double MinEnsembleConfidence { get; set; }

        [Parameter("Адаптивный порог AI", Group = "AI и пороги", DefaultValue = false)]
        ...
        [Parameter("Требовать EV > 0", Group = "AI и пороги", DefaultValue = false)]
        ...
        [Parameter("Пауза после сделки, баров", Group = "Риск", DefaultValue = 0, MinValue = 0, MaxValue = 100)]
        ...
        [Parameter("Макс. спред, доля ATR", Group = "Фильтры", DefaultValue = 0.50, MinValue = 0.01, MaxValue = 2.0, Step = 0.01)]
        public double MaxSpreadAtr { get; set; }

        [Parameter("Макс. спред, доля прибыли", Group = "Фильтры", DefaultValue = 0.25, MinValue = 0.01, MaxValue = 1.0, Step = 0.01)]
        ...
        [Parameter("Метка ордеров", Group = "Журнал", DefaultValue = "QAIv5T")]
        ...
        protected override void OnStart()
        {
            ...
            Print("ТЕСТОВАЯ СБОРКА " + BotVersion + ": фильтры ослаблены, чтобы сделки открывались часто. Цель — проверить " +
                  "механику на демо, а не заработать: бот входит и туда, где сам оценивает сделку как убыточную. " +
                  "Для рабочего режима используйте основную версию.");
            ...
            Print("QuantAI_Universal версия " + BotVersion + " запущена. Потоков: " + _streams.Count + " — " + names +
                  ...
                  " позиций на весь счёт, залог одной сделки ≤ " + F(MaxMarginPercent) + "% свободной маржи. Закрытый рынок (форекс и золото в выходные) поток просто ждёт; крипта торгуется 24/7.");
        }
        ...
        /// <summary>Позиции этого бота на всех инструментах: метка начинается с базовой метки ордеров.</summary>
        internal int TotalOwnPositions()
        {
            string prefix = (string.IsNullOrWhiteSpace(OrderLabel) ? "QAIv5T" : OrderLabel.Trim()) + " ";
            int n = 0;
            foreach (Position p in Positions)
            {
                if (p.Label != null && p.Label.StartsWith(prefix, StringComparison.Ordinal)) n++;
            }
            return n;
        }
    ...
    internal sealed class TradingStream
    {
        ...
        private readonly QuantAI_Universal_V5_6_Test _r;
        ...
        public TradingStream(QuantAI_Universal_V5_6_Test robot, Bars bars, Symbol symbol, string symbolName, TimeFrame timeFrame)
        {
            _r = robot;
            _bars = bars;
            _symbol = symbol;
        ...
        public void Start()
        {
            ...
            _label = (string.IsNullOrWhiteSpace(_r.OrderLabel) ? "QAIv5T" : _r.OrderLabel.Trim()) + " " + ShortTimeFrame();
            ...
            _storageKey = StorageKey("QAIv5T " + _symbolName + " " + _timeFrame);
            if (_r.ResetAiMemory)
            {
        ...
        private void PretrainOnHistory()
        {
            ...
            Log("AI обучен на истории: " + processed + " баров, теневых сделок " + _shadowResolved +
        ...
        private double RegimeFactor(int s, double pTrend) => QuantAI_Universal_V5_6_Test.IsTrendStrategy[s] ? pTrend : 1.0 - pTrend;
        ...
        private void ComputeEnsemble(BarAnalysis a)
        {
            ...
            for (int s = 0; s < QuantAI_Universal_V5_6_Test.StrategyCount; s++)
            {
        ...
        private double ExpectedValueR(double pWin)
        {
            ...
            double remainder = (_r.BreakevenOffsetR + rr) / 2.0;
        ...
        private void OpenShadowTrade(BarAnalysis a)
        {
            ...
            var votes = new double[QuantAI_Universal_V5_6_Test.StrategyCount];
            for (int s = 0; s < QuantAI_Universal_V5_6_Test.StrategyCount; s++)
            {
        ...
        private void ResolveShadowTrades(int i)
        {
            ...
            for (int k = _shadowTrades.Count - 1; k >= 0; k--)
            {
                ...
                for (int s = 0; s < QuantAI_Universal_V5_6_Test.StrategyCount; s++)
                {
                    ...
                    double step = _r.RlLearningRate / 5.0 * t.VoteStrengths[s] * Math.Max(-1.5, Math.Min(1.5, resultR));
        ...
        private void TryEnter(BarAnalysis a, DateTime nowUtc)
        {
            ...
            if (_r.Positions.FindAll(_label, _symbolName).Length >= _r.MaxOpenPositions)
            {
            ...
            if (!TryComputeVolume(type, riskBase, slDistance, out double volume, out double riskMoney, out string sizeNote))
            {
        ...
        /// </summary>
        private bool TryComputeVolume(TradeType type, double riskBase, double slDistance, out double volume, out double riskMoney, out string note)
        {
            volume = 0;
            riskMoney = 0;
            note = "";
            ...
            {
                double minRisk = _symbol.AmountRisked(_symbol.VolumeInUnitsMin, slPips);
            ...
            volume = normalized;
            riskMoney = _symbol.AmountRisked(normalized, slPips);
            return ApplyMarginLimit(type, slPips, ref volume, ref riskMoney, ref note);
        }

        /// <summary>
        /// Залог сделки не должен превышать «Макс. маржа на сделку» от СВОБОДНОЙ маржи. Считает сам брокер
        /// (GetEstimatedMargin) — с его реальным плечом для инструмента: у крипты в ЕС это 1:2, и сделка по
        /// риску 0.5% забирала 40% счёта в залог. Объём уменьшается пропорционально; если не укладывается даже
        /// минимальный лот — вход пропускается с объяснением, а не отклоняется брокером.
        /// </summary>
        private bool ApplyMarginLimit(TradeType type, double slPips, ref double volume, ref double riskMoney, ref string note)
        {
            ...
            double margin = _symbol.GetEstimatedMargin(type, volume);

            if (double.IsNaN(margin) || margin <= 0 || margin <= budget) return true;

            double scaled = _symbol.NormalizeVolumeInUnits(volume * budget / margin, RoundingMode.Down);
            if (scaled < _symbol.VolumeInUnitsMin || budget <= 0)
            {
                note = "Margin: залог минимального лота " + margin.ToString("F2", CultureInfo.InvariantCulture) +
                       " превышает " + F(_r.MaxMarginPercent) + "% свободной маржи (" + freeMargin.ToString("F2", CultureInfo.InvariantCulture) + ")";
                return false;
            }

            double before = volume;
            volume = scaled;
            riskMoney = _symbol.AmountRisked(scaled, slPips);
            note = (note.Length > 0 ? note + "; " : "") + "объём уменьшен " + before.ToString("0.########", CultureInfo.InvariantCulture) +
                   "→" + scaled.ToString("0.########", CultureInfo.InvariantCulture) + " по марже: залог ≤ " + F(_r.MaxMarginPercent) + "% свободной";
            return true;
        }
        ...
        {
            foreach (Position p in _r.Positions.FindAll(_label, _symbolName))
            {
        ...
        public void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            if (p.Label != _label || p.SymbolName != _symbolName) return;
            ...
            if (m.Features != null && m.VoteStrengths != null)
            {
                ...
                for (int s = 0; s < QuantAI_Universal_V5_6_Test.StrategyCount; s++)
                {
                    ...
                    changes.Append(QuantAI_Universal_V5_6_Test.StrategyNames[s]).Append(' ').Append(F(before)).Append("→").Append(F(_realWeights[s])).Append("; ");
                }
        ...
        public void SaveMemory()
        {
            try
            {
                ...
                for (int s = 0; s < QuantAI_Universal_V5_6_Test.StrategyCount; s++) sb.Append(R(_realWeights[s])).Append(';');
                ...
                _r.LocalStorage.SetString(_storageKey, sb.ToString(), LocalStorageScope.Type);
                _r.LocalStorage.Flush(LocalStorageScope.Type);
            }
            catch (Exception ex)
            {
        ...
        private void LoadMemory()
        {
            try
            {
                string text = _r.LocalStorage.GetString(_storageKey, LocalStorageScope.Type);
                ...
                int expected = 1 + QuantAI_Universal_V5_6_Test.StrategyCount + 2 + (2 * NaiveBayesModel.FeatureCount * 3) + 2;
                ...
                int idx = 1;
                for (int s = 0; s < QuantAI_Universal_V5_6_Test.StrategyCount; s++) _realWeights[s] = Math.Max(0.25, Math.Min(3.0, P(parts[idx++], 1.0)));
            ...
            {
                _nbReal.Clear();
                for (int s = 0; s < QuantAI_Universal_V5_6_Test.StrategyCount; s++) _realWeights[s] = 1.0;
        ...
        private string VotesText(BarAnalysis a)
        {
            ...
            for (int s = 0; s < QuantAI_Universal_V5_6_Test.StrategyCount; s++)
            {
                ...
                sb.Append(QuantAI_Universal_V5_6_Test.StrategyNames[s]).Append(v.Direction > 0 ? " ▲" : " ▼").Append(F(v.Strength)).Append("×w").Append(F(EffectiveWeight(s)));
            }
        ...
        private string WeightsText()
        {
            ...
            {
                if (s > 0) sb.Append(", ");
                sb.Append(QuantAI_Universal_V5_6_Test.StrategyNames[s]).Append(' ').Append(F(EffectiveWeight(s)));
            }
            return sb.ToString();
        }
