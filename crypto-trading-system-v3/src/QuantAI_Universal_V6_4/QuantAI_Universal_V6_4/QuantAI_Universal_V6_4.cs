// =====================================================================================================
//  QuantAI_Universal_V6_4 (версия 6.4) — адаптивный AI-cBot для cTrader (API 4.x / cTrader.Automate)
//
//  ИСТОРИЯ ВЕРСИЙ
//   6.4 — оценка истории по живому спреду. Бары cTrader не хранят спред, поэтому история оценивается по спреду
//         в момент запуска. Запуск на закрытом рынке (выходные, перерыв) брал последний, часто расширенный спред —
//         и стратегии получали ложный «✗ убыточен» (LTCUSD: все пять по −0.5R). Теперь, если рынок при запуске был
//         закрыт, после открытия и 20 живых тиков поток заново проходит историю по текущему спреду. Спред, по
//         которому оценена история, пишется в журнал.
...
namespace cAlgo.Robots
{
    ...
    internal sealed class BarAnalysis
    {
        ...
        public readonly StrategyVote[] Votes = new StrategyVote[QuantAI_Universal_V6_4.StrategyCount];
    ...
    internal sealed class TradingStream
    {
        ...
        private readonly QuantAI_Universal_V6_4 _r;
        ...
        private OnlineLogistic _regimeModel = new OnlineLogistic(5);

        // Запуск на закрытом рынке: спред для истории ненадёжен — после открытия история переоценивается.
        private bool _pretrainedWhileClosed;
        private int _liveTicks;
        private double _pretrainSpread;
        ...
        private readonly EdgeTracker[] _edge = new EdgeTracker[QuantAI_Universal_V6_4.StrategyCount];
        ...
        public TradingStream(QuantAI_Universal_V6_4 robot, Bars bars, Symbol symbol, string symbolName, TimeFrame timeFrame)
        {
            _r = robot;
            _bars = bars;
            _symbol = symbol;
        ...
        public void Start()
        {
            ...
            _legacyLabels = new string[QuantAI_Universal_V6_4.LegacyLabels.Length];
            for (int k = 0; k < _legacyLabels.Length; k++) _legacyLabels[k] = QuantAI_Universal_V6_4.LegacyLabels[k] + " " + ShortTimeFrame();

            for (int s = 0; s < QuantAI_Universal_V6_4.StrategyCount; s++) _edge[s] = new EdgeTracker(EvidenceWindow);
            ...
            _storageKey = StorageKey(QuantAI_Universal_V6_4.MemoryPrefix + " " + _symbolName + " " + _timeFrame);
            if (_r.ResetAiMemory)
            {
            ...
            EnsureHistory();
            _pretrainedWhileClosed = !IsMarketOpenSafe();
            PretrainOnHistory();
            if (_pretrainedWhileClosed)
            {
                Log("Рынок " + _symbolName + " сейчас закрыт — спред для оценки истории ненадёжен. После открытия рынка история будет переоценена по живому спреду.");
            }
        ...
        private bool IsMarketOpenSafe()
        {
            try
            {
                return _symbol.MarketHours.IsOpened();
            }
            catch (Exception)
            {
                return true;
        ...
        /// <summary>
        /// Повторное обучение на истории по текущему спреду: теневые сделки, счета стратегий, Naive Bayes по теневым
        /// сделкам и модель режима строятся заново. Знания из реальных сделок (память AI) не трогаются.
        /// </summary>
        private void RetrainOnLiveSpread()
        {
            double before = _pretrainSpread;
            _shadowTrades.Clear();
            _regimeSamples.Clear();
            _nbShadow.Clear();
            _regimeModel = new OnlineLogistic(5);
            for (int s = 0; s < QuantAI_Universal_V6_4.StrategyCount; s++)
            {
                _edge[s] = new EdgeTracker(EvidenceWindow);
                _shadowMultipliers[s] = 1.0;
            }
            _shadowResolved = 0;
            _shadowWins = 0;
            _shadowSumR = 0;
            ...
            Log("Рынок открыт — переоценка истории по живому спреду " + FormatSpread(CurrentSpread()) + " (при запуске был " + FormatSpread(before) + ").");
            PretrainOnHistory();
        }

        private string FormatSpread(double spread) =>
            spread.ToString("F" + _symbol.Digits, CultureInfo.InvariantCulture) + " = " + (spread / _symbol.PipSize).ToString("F1", CultureInfo.InvariantCulture) + " п.";

        private void OnSymbolTick(SymbolTickEventArgs args)
        {
            if (_pretrainedWhileClosed && _liveTicks < 1000) _liveTicks++;
        ...
        private void OnBarOpened(BarOpenedEventArgs args)
        {
            ...
            if (_pretrainedWhileClosed && _liveTicks >= 20 && IsMarketOpenSafe())
            {
                _pretrainedWhileClosed = false;
                RetrainOnLiveSpread();
            }
        ...
        private void PretrainOnHistory()
        {
            ...
            Log("AI обучен на истории: " + processed + " баров. Теневые сделки ансамбля (по правилам реальных: частичное " +
                  ...
                  " на сделку. Спред для оценки: " + FormatSpread(_pretrainSpread) + ". Обновлений модели режима: " + _regimeModel.Updates + ".");
            Log(CommissionText());
            Log(EdgeReport());
        }
        ...
        private BarAnalysis ProcessClosedBar(int i)
        {
            ...
            bool rolloverPause = !QuantAI_Universal_V6_4.IsCrypto(_symbolName) && _r.IsSessionBlocked(entryTime);
        ...
        private double RegimeFactor(int s, double pTrend) => QuantAI_Universal_V6_4.IsTrendStrategy[s] ? pTrend : 1.0 - pTrend;
        ...
        private void OpenShadowTrade(BarAnalysis a)
        {
            ...
            t.VoteStrengths = new double[QuantAI_Universal_V6_4.StrategyCount];
            for (int s = 0; s < QuantAI_Universal_V6_4.StrategyCount; s++)
            {
        ...
        {
            EdgeStats st = Edge(s);
            string name = QuantAI_Universal_V6_4.StrategyNames[s];
        ...
        private void TryEnter(BarAnalysis a, DateTime nowUtc)
        {
            ...
            if (!QuantAI_Universal_V6_4.IsCrypto(_symbolName) && _r.IsSessionBlocked(nowUtc))
            {
            ...
            var tiers = new StrategyTier[QuantAI_Universal_V6_4.StrategyCount];
            var allowed = new bool[QuantAI_Universal_V6_4.StrategyCount];
            ...
            var votes = new double[QuantAI_Universal_V6_4.StrategyCount];
            for (int s = 0; s < QuantAI_Universal_V6_4.StrategyCount; s++)
            {
                if (allowed != null && !allowed[s]) continue;
        ...
        public void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            if (m.Features != null && m.VoteStrengths != null)
            {
                ...
                for (int s = 0; s < QuantAI_Universal_V6_4.StrategyCount; s++)
                {
                    ...
                    changes.Append(QuantAI_Universal_V6_4.StrategyNames[s]).Append(' ').Append(F(before)).Append("→").Append(F(_realWeights[s])).Append("; ");
                }
        ...
        public void SaveMemory()
        {
            try
            {
                ...
                for (int s = 0; s < QuantAI_Universal_V6_4.StrategyCount; s++) sb.Append(R(_realWeights[s])).Append(';');
        ...
        private void LoadMemory()
        {
            try
            {
                ...
                int expected = 1 + QuantAI_Universal_V6_4.StrategyCount + 2 + (2 * NaiveBayesModel.FeatureCount * 3) + 2;
                ...
                int idx = 1;
                for (int s = 0; s < QuantAI_Universal_V6_4.StrategyCount; s++) _realWeights[s] = Math.Max(0.25, Math.Min(3.0, P(parts[idx++], 1.0)));
            ...
            {
                _nbReal.Clear();
                for (int s = 0; s < QuantAI_Universal_V6_4.StrategyCount; s++) _realWeights[s] = 1.0;
        ...
        private string VotesText(BarAnalysis a)
        {
            ...
            for (int s = 0; s < QuantAI_Universal_V6_4.StrategyCount; s++)
            {
                ...
                sb.Append(QuantAI_Universal_V6_4.StrategyNames[s]).Append(v.Direction > 0 ? " ▲" : " ▼").Append(F(v.Strength)).Append("×w").Append(F(EffectiveWeight(s)));
            }
        ...
        private string WeightsText()
        {
            ...
            {
                if (s > 0) sb.Append(", ");
                sb.Append(QuantAI_Universal_V6_4.StrategyNames[s]).Append(' ').Append(F(EffectiveWeight(s)));
            }
            return sb.ToString();
        }
