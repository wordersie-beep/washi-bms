...
public sealed class ExecutionEngine
{
        ...
        Action<string> log)
    {
        ...
        for (int attempt = 0; attempt <= _config.MaxOrderRetries; attempt++)
        {
            ...
            // Повтор допустим ТОЛЬКО когда точно известно, что ордера нет: нет котировки,
            // реквота, временный отказ с ответом сервера. При таймауте и обрыве связи ответа
            // нет, и ордер мог быть принят — повтор в этом случае открывает вторую позицию
            // того же размера. Повторять неизвестность нельзя, её можно только выяснить, и
            // выясняет её сверка с брокером.
            if (result.OutcomeUnknown)
            {
                log?.Invoke($"повтор отменён: исход неизвестен ({result.Error}). " +
                            "Второй ордер мог бы стать дублем — состояние выяснит сверка.");
                break;
            }
