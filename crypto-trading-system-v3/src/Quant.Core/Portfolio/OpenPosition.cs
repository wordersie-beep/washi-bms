...
public sealed class OpenPosition
{
    ...
    /// (см. <see cref="SymbolSpec.MoneyPerPricePerUnit"/>).
    ///
    /// Хранится на позиции, а не берётся из спецификации при каждом расчёте: портфельный
    /// слой не должен разыскивать инструменты, чтобы посчитать риск. Значение обновляется
    /// на сопровождении позиции — курс валюты счёта к котируемой меняется со временем.
    ///
    /// Единица по умолчанию означает «котируемая валюта совпадает с валютой счёта» —
    /// безопасно для обычного случая и честно для остальных, пока значение не обновлено.
    /// </summary>
    public double MoneyPerPricePerUnit { get; set; } = 1.0;
