// =====================================================================================================
//  QuantAI_Universal_V5_9 (версия 5.9) — адаптивный AI-cBot для cTrader (API 4.x / cTrader.Automate)
//
//  ИСТОРИЯ ВЕРСИЙ
...
namespace cAlgo.Robots
{
    ...
    internal sealed class BarAnalysis
    {
        ...
        public readonly StrategyVote[] Votes = new StrategyVote[QuantAI_Universal_V5_9.StrategyCount];
    ...
    public class QuantAI_Universal_V5_9 : Robot
    {
        ...
        private const int MaxStreams = 12;

        /// <summary>Метка ордеров по умолчанию и метка тестовых версий 5.3–5.8, чьи позиции подхватываются.</summary>
        internal const string DefaultLabel = "QAIv5";
        internal const string LegacyLabel = "QAIv5T";

        // ----- Параметры: потоки ------------------------------------------------------------------

        [Parameter("Доп. таймфреймы", Group = "Потоки", DefaultValue = "m30, h1")]
        public string ExtraTimeFrames { get; set; }

        [Parameter("Доп. инструменты", Group = "Потоки", DefaultValue = "XAUUSD, BTCUSD")]
        public string ExtraSymbols { get; set; }

        [Parameter("Макс. позиций всего", Group = "Потоки", DefaultValue = 3, MinValue = 1, MaxValue = 20)]
        ...
        [Parameter("Только стратегии с доказанным плюсом", Group = "Доказательность", DefaultValue = true)]
        public bool EvidenceGate { get; set; }
        ...
        [Parameter("Торговые часы не-крипто с (UTC)", Group = "Фильтры", DefaultValue = "07:00")]
        public string TradeHoursStart { get; set; }

        [Parameter("Торговые часы не-крипто до (UTC)", Group = "Фильтры", DefaultValue = "19:00")]
        ...
        protected override void OnStart()
        {
            ...
            Print("QuantAI_Universal " + BotVersion + " — рабочий режим. Сделка открывается, только если стратегия доказала плюс " +
                  "на теневых сделках (со спредом и комиссией, по тем же правилам сопровождения) и ожидание сделки > 0. " +
                  "Сделок меньше, чем в тестовых версиях, — это и есть отбор. Прибыль не гарантируется: доказательства " +
                  "берутся из прошлого, рынок может измениться.");
            ...
            Print("QuantAI_Universal версия " + BotVersion + " запущена. Потоков: " + _streams.Count + " — " + names +
                  ...
                  " позиций на весь счёт и " + MaxPositionsPerSymbol + " на инструмент, залог одной сделки ≤ " + F(MaxMarginPercent) +
                  "% свободной маржи. Форекс и золото — " + (_tradeHoursEnabled ? TradeHoursStart + "–" + TradeHoursEnd + " UTC" : "в любое время работы рынка") +
                  ", крипта — 24/7.");
        }
        ...
        private void BuildStreams()
        {
            ...
            {
                if (ContainsIgnoreCase(symbols, raw)) continue;
                if (!Symbols.Exists(raw))
                {
            ...
            foreach (string symbolName in symbols)
            {
                foreach (TimeFrame tf in frames)
                {
                    ...
                    {
                        Print("Потоков больше " + MaxStreams + " — остальные пропущены, чтобы не перегрузить облако.");
                        return;
                    }
        ...
        private void ValidateGlobalSettings()
        {
            ...
            _tradeHoursEnabled = TryParseTime(TradeHoursStart, out _hoursStart) & TryParseTime(TradeHoursEnd, out _hoursEnd);
            if (!_tradeHoursEnabled)
            {
                Print("ВНИМАНИЕ: не удалось прочитать торговые часы ('" + TradeHoursStart + "'–'" + TradeHoursEnd +
                      "'). Формат ЧЧ:ММ. Форекс и золото будут торговаться в любые часы работы рынка.");
            }
            ...
            Print("Пороги (все из параметров UI): режим ≥ " + F(MinRegimeConfidence) +
                  ...
                  (EvidenceGate
                      ? " Торгуются только стратегии с доказанным плюсом: не меньше " + EvidenceMinTrades + " теневых сделок, среднее − " +
                        F(EvidenceSigma) + "σ > 0 и плюс в обеих половинах периода."
                      : " Отбор стратегий по доказательствам ВЫКЛЮЧЕН."));
        }
        ...
        /// <summary>Позиция этого бота: метка «база ТФ» или метка тестовых версий «QAIv5T ТФ».</summary>
        internal bool IsOwnLabel(string label) =>
            label != null &&
            (label.StartsWith(BaseLabel + " ", StringComparison.Ordinal) || label.StartsWith(LegacyLabel + " ", StringComparison.Ordinal));
    ...
    internal sealed class TradingStream
    {
        ...
        private readonly QuantAI_Universal_V5_9 _r;
        ...
        private readonly EdgeTracker[] _edge = new EdgeTracker[QuantAI_Universal_V5_9.StrategyCount];
        ...
        public TradingStream(QuantAI_Universal_V5_9 robot, Bars bars, Symbol symbol, string symbolName, TimeFrame timeFrame)
        {
            _r = robot;
            _bars = bars;
            _symbol = symbol;
        ...
        public void Start()
        {
            ...
            _legacyLabel = QuantAI_Universal_V5_9.LegacyLabel + " " + ShortTimeFrame();

            for (int s = 0; s < QuantAI_Universal_V5_9.StrategyCount; s++) _edge[s] = new EdgeTracker(EvidenceWindow);
            ...
            _storageKey = StorageKey(QuantAI_Universal_V5_9.DefaultLabel + " " + _symbolName + " " + _timeFrame);
            if (_r.ResetAiMemory)
            {
        ...
        private bool IsMine(Position p) => p.SymbolName == _symbolName && (p.Label == _label || p.Label == _legacyLabel);
        ...
        private BarAnalysis ProcessClosedBar(int i)
        {
            ...
            if (_r.InTradingHours(_symbolName, EntryTime(i))) OpenStrategyShadows(a);
        ...
        private double RegimeFactor(int s, double pTrend) => QuantAI_Universal_V5_9.IsTrendStrategy[s] ? pTrend : 1.0 - pTrend;
        ...
        private void ComputeEnsemble(BarAnalysis a, bool[] allowed = null)
        {
            ...
            for (int s = 0; s < QuantAI_Universal_V5_9.StrategyCount; s++)
            {
        ...
        private void OpenShadowTrade(BarAnalysis a)
        {
            ...
            t.VoteStrengths = new double[QuantAI_Universal_V5_9.StrategyCount];
            for (int s = 0; s < QuantAI_Universal_V5_9.StrategyCount; s++)
            {
        ...
        {
            EdgeStats st = Edge(s);
            string name = QuantAI_Universal_V5_9.StrategyNames[s];
            if (st.Count < _r.EvidenceMinTrades) return name + " мало данных (" + st.Count + " из " + _r.EvidenceMinTrades + ")";
            return name + " " + SignedR(st.Mean) + " (n=" + st.Count + ", граница " + SignedR(st.LowerBound) +
                   ", половины " + SignedR(st.OlderMean) + "/" + SignedR(st.NewerMean) + ")" + (st.Proven(_r.EvidenceMinTrades) ? " ✓" : " ✗");
        }

        /// <summary>Какие стратегии этого потока доказали плюс — печатается при старте и в сводке.</summary>
        private string EdgeReport()
        {
            var sb = new StringBuilder("Проверка стратегий на теневых сделках (последние до " + EvidenceWindow + ", после спреда и комиссии): ");
            var proven = new List<string>();
            for (int s = 0; s < QuantAI_Universal_V5_9.StrategyCount; s++)
            {
                if (s > 0) sb.Append("; ");
                sb.Append(EdgeText(s));
                if (IsProven(s)) proven.Add(QuantAI_Universal_V5_9.StrategyNames[s]);
            }

            if (!_r.EvidenceGate)
            {
                sb.Append(". Отбор по доказательствам выключен — торгуются все стратегии.");
            }
            else if (proven.Count == 0)
            {
                sb.Append(". Ни одна стратегия не доказала плюс — поток НЕ торгует и продолжает учиться на теневых сделках.");
            }
            else
            {
                sb.Append(". Торгуются: ").Append(string.Join(", ", proven)).Append('.');
            }
            return sb.ToString();
        }
        ...
        private string CommissionText()
        {
            ...
            if (perUnit <= 0)
            {
                return "Комиссия по " + _symbolName + ": не найдена в сделках счёта (для крипты у Pepperstone её обычно нет) — " +
        ...
        private void TryEnter(BarAnalysis a, DateTime nowUtc)
        {
            ...
            if (_r.IsSessionBlocked(nowUtc))
            {
                RegisterSkip(SkipReason.SessionBlock, "межсессионное окно " + _r.SessionBlockStart + "–" + _r.SessionBlockEnd + " UTC (расширение спреда)", a);
                return;
            }
            ...
            if (!_r.InTradingHours(_symbolName, nowUtc))
            {
                RegisterSkip(SkipReason.OffHours, "вне торговых часов " + _r.TradeHoursStart + "–" + _r.TradeHoursEnd +
                    " UTC: форекс и золото торгуются только в ликвидные часы Лондона и Нью-Йорка", a);
                return;
            }
            ...
            // Отбор по доказательствам: голоса стратегий без доказанного плюса не учитываются вовсе.
            bool[] allowed = null;
            string edgeNote = "";
            if (_r.EvidenceGate)
            {
                allowed = new bool[QuantAI_Universal_V5_9.StrategyCount];
                var provenText = new StringBuilder();
                var rejectedText = new StringBuilder();
                for (int s = 0; s < QuantAI_Universal_V5_9.StrategyCount; s++)
                {
                    ...
                    {
                        allowed[s] = true;
                        if (provenText.Length > 0) provenText.Append("; ");
                        provenText.Append(EdgeText(s));
                    }
                    else
                    {
                        if (rejectedText.Length > 0) rejectedText.Append("; ");
                        rejectedText.Append(EdgeText(s));
                    }
                }

                if (provenText.Length == 0)
                {
                    RegisterSkip(SkipReason.NoEdge, "сигнал дают стратегии без доказанного плюса: " + rejectedText, a);
                    return;
                }
                ...
                if (a.EnsembleDirection == 0)
                {
                    RegisterSkip(SkipReason.NoSignal, "доказанные стратегии не дают общего направления: " + provenText, a);
                    return;
                }
                edgeNote = " | доказано: " + provenText;
            }
            ...
            if (!TryComputeVolume(type, riskBase, slDistance, costR, out double volume, out double riskMoney, out string sizeNote))
            {
            ...
            var votes = new double[QuantAI_Universal_V5_9.StrategyCount];
            for (int s = 0; s < QuantAI_Universal_V5_9.StrategyCount; s++)
            {
                if (allowed != null && !allowed[s]) continue;
        ...
        /// </summary>
        private bool TryComputeVolume(TradeType type, double riskBase, double slDistance, double costR, out double volume, out double riskMoney, out string note)
        {
            volume = 0;
            riskMoney = 0;
            note = "";

            double effectiveRisk = Math.Min(_r.RiskPercent, _r.MaxRiskCapPercent);
            ...
            if (normalized < _symbol.VolumeInUnitsMin)
            {
                ...
                if (minRisk > 0 && minRiskPercent <= _r.MaxRiskCapPercent)
                {
                ...
                note = "Account Too Small: минимальный лот рискует " + minRisk.ToString("F2", CultureInfo.InvariantCulture) + " (" +
                       minRiskPercent.ToString("F1", CultureInfo.InvariantCulture) + "% счёта) при потолке " + F(_r.MaxRiskCapPercent) +
                       "%. Нужен счёт от " + (minRisk / (_r.MaxRiskCapPercent / 100.0)).ToString("F0", CultureInfo.InvariantCulture) + ".";
                return false;
            }
        ...
        public void OnPositionClosed(PositionClosedEventArgs args)
        {
            ...
            if (m.Features != null && m.VoteStrengths != null)
            {
                ...
                for (int s = 0; s < QuantAI_Universal_V5_9.StrategyCount; s++)
                {
                    ...
                    changes.Append(QuantAI_Universal_V5_9.StrategyNames[s]).Append(' ').Append(F(before)).Append("→").Append(F(_realWeights[s])).Append("; ");
                }
        ...
        public void SaveMemory()
        {
            try
            {
                ...
                for (int s = 0; s < QuantAI_Universal_V5_9.StrategyCount; s++) sb.Append(R(_realWeights[s])).Append(';');
        ...
        private void LoadMemory()
        {
            try
            {
                ...
                int expected = 1 + QuantAI_Universal_V5_9.StrategyCount + 2 + (2 * NaiveBayesModel.FeatureCount * 3) + 2;
                ...
                int idx = 1;
                for (int s = 0; s < QuantAI_Universal_V5_9.StrategyCount; s++) _realWeights[s] = Math.Max(0.25, Math.Min(3.0, P(parts[idx++], 1.0)));
            ...
            {
                _nbReal.Clear();
                for (int s = 0; s < QuantAI_Universal_V5_9.StrategyCount; s++) _realWeights[s] = 1.0;
        ...
        private string VotesText(BarAnalysis a)
        {
            ...
            for (int s = 0; s < QuantAI_Universal_V5_9.StrategyCount; s++)
            {
                ...
                sb.Append(QuantAI_Universal_V5_9.StrategyNames[s]).Append(v.Direction > 0 ? " ▲" : " ▼").Append(F(v.Strength)).Append("×w").Append(F(EffectiveWeight(s)));
            }
        ...
        private string WeightsText()
        {
            ...
            {
                if (s > 0) sb.Append(", ");
                sb.Append(QuantAI_Universal_V5_9.StrategyNames[s]).Append(' ').Append(F(EffectiveWeight(s)));
            }
            return sb.ToString();
        }
