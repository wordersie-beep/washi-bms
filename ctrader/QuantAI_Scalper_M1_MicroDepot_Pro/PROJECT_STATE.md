...
- Тесты: 103/103 (Naive Bayes, в т.ч. признак без разброса, тиковый движок, калибратор, медиана спреда, комиссия, симулятор исходов, сквиз/свип, деление объёма).
- Брокер пользователя: Pepperstone (cTrader, облако, EU 1:30). Нужен счёт Razor; на Standard фильтр спреда блокирует входы.
- Первый журнал v1.2.0 (демо, 25.09 19:23 UTC): баланс 49 092 EUR, спред 0.0 (Razor), 3000 баров → 5990 примеров,
  базовый win-rate 27 %, средние признаков по классам почти равны (EMA −0.027/0.010, RSI −0.039/0.014) → AI слабый,
  уверенность около 50 %; порог снижен до 0.50.
