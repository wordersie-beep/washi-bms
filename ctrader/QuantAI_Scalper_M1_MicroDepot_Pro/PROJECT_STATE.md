...
- Готовый `release/QuantAI_Scalper_M1_MicroDepot_Pro.algo` (API 1.0.9): ставится с телефона, экземпляр сам
  открывается на EURUSD M1, все 59 параметров выставлены (проверено по метаданным `.algo`).
...
- Тесты: 89/89 (Naive Bayes, тиковый движок, калибратор, медиана спреда, симулятор исходов, сквиз/свип, деление объёма).
...
- Чистые классы без зависимостей от cTrader: `GaussianNaiveBayes`, `TickVelocityEngine`, `TickCalibrator`,
  `SpreadTracker`, `OutcomeSimulator`, `VolmanSetups`, `SweepDetector`, `VolumeMath`, `AiFeatures`.
...
## Открытые задачи
1. Демо-счёт в облаке 1–2 недели с параметрами по умолчанию; сверить журнал `SKIP`/`ENTRY`/`CLOSED`.
2. Бэктест EURUSD M1, «Tick data from server», 3–6 месяцев; по строкам `AI L/S` в журнале подобрать `Min Confidence`.
